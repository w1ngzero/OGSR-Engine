...
void CKinematics::Load(const char* N, IReader* data, const u32 dwFlags)
{
    ...
    else if (SourceMdl::psSourceSkeletonMode && LoadSourceSkeleton(N))
    {
        ...
        // [importer-v2] — версия импортёра Source. Если в логе НЕТ этой метки, запущенный
        // xrEngine.exe собран из старых исходников (это помогает отличить новую сборку от старой).
        Msg("[SourceSkeleton] imported skeleton for '%s' (%d bone(s)) [importer-v2]", N, (int)bones->size());
