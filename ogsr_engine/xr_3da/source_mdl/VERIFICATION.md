...
#### Уточнение (hold-кости, найдено и исправлено при сборке)
В OMF-формате у всех костей последовательности ОДИН общий счётчик кадров (len). Первая версия
`BuildXRayMotionsOMF` строила hold-трек не-анимированной кости с ОДНИМ identity-кадром, а писала
`len` ключей — из массива на 1 элемент (выход за границы + «уезд» кости на кадрах >0). Исправлено:
`MakeHoldTrack(bone, len)` теперь генерирует ровно `len` identity-кадров. Верер (round-trip)
усилен проверкой, что hold-кости декодируются в identity на ВСЕХ кадрах; v_knife и v_akilo47 —
0 плохих кадров.

## Round 16 — финальная офлайн-верификация связки анимаций (зазоры закрыты)

Проверено и закрыто последнее отложенное в этому раунду:

1. **Hold-track после фикса.** Собран заново и прогнан round-trip на v_knife и v_akilo47:
   - v_knife (40 seq / 198 bone, OMF 5 000 608 B): плохих кадров 0, maxRotAttErr 2.6e-05
   - v_akilo47 (67 seq / 215 bone, OMF 3 853 177 B): плохих кадров 0, maxRotAttErr 2.4e-05
   Верер теперь ДОПОЛНИТЕЛЬНО проверяет, что hold-кости (не-анимированные на сегменте)
   декодируются в identity на ВСЕХ кадрах, а не только на первом — поймал бы прежний OOB.
   `source_mdl_anim_to_xray.cpp` компилируется чисто против стабов (stub_check, EXIT=0).

2. **Разрешение имён/длительностей последовательностей** (закрыт ранее «зависший» вопрос по
   v_akilo47 `inspect`): имена и длительности декодируются корректно из хедера .mdl, напр.
   v_knife `inspect` = 154 кадра @30fps; переходные/оффсетные сегменты — 1 кадр (легитимно).
   ОМF содержит имя каждое seq, поэтому игра адресует анимации по имени.

Все изменения синхронизированы в `xray_source_patch/xr_3da/source_mdl`, 
`xray_source_patch/Layers/xrRender` и `xray_source_patch/github_upload/ogsr_engine/...`.
Zip: `xray_source_patch/github_upload_ogsr_engine.zip` = 161 879 б (Sep 6 06:29).

### Что остаётся за пользователем (проверка на экране)
Офлайн-связка (Source .mdl → OMF → `shared_motions::create` → `m_Motions`) готова и зелёная.
Далее нужно собрать OGSR и убедиться в игре:
- кастомный .mdl (напр. v_knife/v_akilo47) грузится как `MT_SKELETON_ANIM` → создаётся
